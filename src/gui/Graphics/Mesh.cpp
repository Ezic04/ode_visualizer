...
bool Mesh::initializeGL(const std::vector<float> &vertices, const std::vector<unsigned int> &indices,
  ...
  glBufferData(GL_ARRAY_BUFFER, sizeof(std::array<float, 3>) * instances.size(), instances.data()->data(),
               GL_DYNAMIC_DRAW);
